...
//表の出力
    ...
    if ($_POST['name'] == "全員"){
        if ( ! $_POST['date1']){
            $sql = $pdo->prepare("SELECT * FROM kintai ORDER BY date");
        }elseif ( ! $_POST['date2']){
        $sql = $pdo->prepare("SELECT * FROM kintai WHERE date = :pdate1 ORDER BY date");
...
if($ct == 0){
    die("データがありません<p><A href='index.html'>ホーム</A></p>");
}
//出勤日カウント
if ( ! $_POST['name'] == "全員"){
...
//出勤日出力
    ...
    if( ! array_key_exists("欠勤", $count)){
        print "この期間の欠勤日数は0日です。 <br />";
    }else{
        print "この期間の欠勤日数は".$count["欠勤"]."日です。 <br />";
    }
    if( ! array_key_exists("有給", $count)){
        print "この期間の有給日数は0日です。 <br />";
    }else{
        print "この期間の有給日数は".$count["有給"]."日です。 <br />";
    }
